...
//FUNCTION THAT READS THE KEYPADS ROWS AND COLUMNS CONSTATNLY FOR A BUTTON BEING PRESSED, IF A ROW AND COLUMN ARE CONNECTED THE FUNCTION RETURNS THE KEY VALUE WHERE THE CONNECTION WAS MADE (USED FOR DEBUGGING)
uint8_t read_keypad(){
    ...
    for (col=0; col<3; col++){          //looping through each column to see if a key is being pressed
        P6->DIR &= ~(BIT4|BIT5|BIT6);   //set columns to inputs (all columns on P6.4-4.6 set to 0b00000000)
...
//THIS FUNCTION PRINTS THE VALUE OF THE KEY BEING PRESSED, PRINTS THE PROPER VALUE FOR *, 0, AND #
void print_key(){
...
//THIS FUNCTION IS FOR THE LED BRIGHTNESS, IT IS CALLED IN THE MENUS.C SOURCE FILE AND RETURNS A 0-3 DIGIT NUMBER ONCE THE # IS PRESSED
uint8_t keypad_pin(){
